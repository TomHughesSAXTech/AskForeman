// Alternative n8n Google Search Node Code
// This version tries GET first, then falls back to POST if needed
...
try {
  ...
  // Try Method 1: GET with query parameters (recommended)
  try {
    ...
    console.log(`Attempting GET request to: ${azureFunctionUrl}`);
    ...
    console.log('GET request failed, trying POST method:', getError.message);
    
    // Try Method 2: POST with function key in URL and query in body
    try {
      response = await this.helpers.httpRequest({
        method: 'POST',
        url: `${azureFunctionUrl}?code=${functionKey}`,
        headers: {
        ...
        body: {
          q: enhancedQuery,
          query: enhancedQuery,  // Send both in case the function expects either
          limit: 5
        },
        json: true,
        timeout: 10000
  ...
  response.results.forEach((item, index) => {
    // Aggressive URL cleaning to remove all trailing punctuation
  ...
  if (error.message && error.message.includes('404')) {
    return `Search service not found. Please verify the Azure Function is deployed at: ${azureFunctionUrl}`;
  } else if (error.message && error.message.includes('timeout')) {
    return `Search request timed out. Please try again.`;
  } else if (error.message && error.message.includes('401') || error.message && error.message.includes('403')) {
    return `Authentication failed. The function key may be incorrect or expired. Please verify in Azure Portal.`;
  } else {
