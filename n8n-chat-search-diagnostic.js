// DIAGNOSTIC VERSION - To debug why documents aren't being found
  ...
  try {
    ...
    if (response.value && response.value.length > 0) {
      ...
      if (response.value && response.value.length > 0) {
        ...
        return `No documents found in the entire index. The index appears to be empty.`;
      }
    }
...
// For non-document queries, just return a simple message
return `Query type not recognized. Please ask "what documents do you have?" to list available documents.`;
